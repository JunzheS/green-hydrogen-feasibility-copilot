...
# ─── PERSISTENT LANDING PAGE (always visible) ───
...
# Recent assessment summary (shown when an assessment exists)
if st.session_state.get("assessment_complete") and st.session_state.get("report"):
    ...
    cap_central = capex.get("total", {}).get("central_eur_m", 0)
    lcoh_c = lcoh.get("central_eur_per_kg", 0)

    st.markdown("#### Recent Assessment")
    rc1, rc2, rc3, rc4 = st.columns(4)
    with rc1: st.metric("Project", f"{q.get('capacity_mw','')} MW {q.get('technology','')}")
    with rc2: st.metric("Gate", gate)
    with rc3: st.metric("CAPEX", f"EUR {cap_central:.0f}M")
    with rc4: st.metric("LCOH", f"EUR {lcoh_c:.2f}/kg")
    st.markdown("Navigate to **Assessment Report**, **Risk Assessment**, or **CAPEX & LCOH** for full details.")
    st.divider()
